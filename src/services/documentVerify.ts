...
import {buildChipDocumentViewModel} from './chipDocumentModel';
import {REGISTRY_CHAIN_ID} from './registryChain';
...
function verifyDg1Integrity(chipDoc: any): void {
  const dg1Group = chipDoc.dataGroups?.find((dg: any) => dg.groupNumber === 1);
  if (!dg1Group) {
    throw new DocumentVerifyError('DG1 missing from chip data');
  }
  const dg1HashFromSod =
    chipDoc.sod.encapContentInfo.eContent.dataGroupHashValues.values[1];
  if (!dg1HashFromSod) {
...
async function verifySodSignature(chipDoc: any): Promise<void> {
  if (!isIDSupported(chipDoc)) {
    throw new DocumentVerifyError(
      'SOD signature algorithm is not supported on this device',
    );
  }

  const tbs = extractTBS(chipDoc);
  if (!tbs) {
    throw new DocumentVerifyError(
      'Could not extract DSC certificate from chip data',
    );
  }

  const signedAttrsBytes =
    chipDoc.sod.signerInfo.signedAttrs.bytes.toUInt8Array();
  const signatureBytes = chipDoc.sod.signerInfo.signature.toUInt8Array();
  const hashAlg = normalizeHashAlgorithm(
    chipDoc.sod.signerInfo.digestAlgorithm,
  );
  const digest = HASH_FN[hashAlg](signedAttrsBytes);

  const sodSigType = getSodSignatureAlgorithmType(chipDoc);
  const sigName = String(
    chipDoc.sod.signerInfo.signatureAlgorithm.name || '',
  ).toLowerCase();
  ...
  if (sodSigType === 'ECDSA') {
    ...
    if (!verifyECDSASignature(digest, signatureBytes, publicKey)) {
      throw new DocumentVerifyError(
        'SOD signature verification failed (ECDSA)',
      );
    }
    return;
  }
...
/**
 * Plain registry verify at NFC scan.
 * Loads CSCA/DSC chain from zkPassport registry and checks DSC + SOD + DG1 integrity.
 */
export async function plainVerifyDocument(
  dg1Base64: string,
  sodBase64: string,
): Promise<PlainVerifyResult> {
  ...
  let chipDoc: any;
  try {
    chipDoc = buildChipDocumentViewModel({dg1: dg1Base64, sod: sodBase64});
  } catch (err: any) {
    throw new DocumentVerifyError(
      err?.message || 'Could not parse document chip data',
    );
  }

  const registry = new RegistryClient({chainId: REGISTRY_CHAIN_ID});
  const {packaged: packagedCerts, certRoot} =
    await resolvePackagedCertificates(registry);

  const csca = await getCscaForPassportAsync(
    chipDoc.sod.certificate,
    packagedCerts.certificates as PackagedCertificate[],
  );
  if (!csca) {
    throw new DocumentVerifyError(
      'Could not match CSCA from certificate registry',
    );
  }

  const dscOk = await verifyDscSignature(chipDoc.sod.certificate, csca);
  if (!dscOk) {
    throw new DocumentVerifyError(
      'DSC signature verification failed against registry CSCA',
    );
  }

  await verifySodSignature(chipDoc);
  verifyDg1Integrity(chipDoc);
