...
export interface ChipDocumentInput {
  dg1: string;
  sod: string;
  dg2?: string;
}

/** ICAO chip view model for plain verify (field names match @zkpassport/utils). */
export function buildChipDocumentViewModel(pd: ChipDocumentInput): any {
  ...
  const dg2Group = pd.dg2
    ? (() => {
        ...
        return {
          ...
          name: 'DG2',
          hash: dg2FromSod
            ? dg2FromSod.toNumberArray()
            : Array.from(sha256(raw.toUInt8Array())),
